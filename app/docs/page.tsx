...
function renderItem(item: DocItem, idx: number) {
  switch (item.type) {
    ...
    case 'h3':
      return (
        <h3
          key={idx}
          id={item.id}
          className="font-display font-semibold text-white text-[1.05rem] mt-8 mb-2 scroll-mt-20"
        >
          {item.title}
        </h3>
      )
    ...
    case 'props':
      return (
        <div key={idx} className="border border-[#1E1E1E] my-4 bg-[#080808]">
          {item.header && (
    ...
    case 'grid': {
      ...
      return (
        <div key={idx} className={`grid ${gridClass} gap-2 my-3`}>
          {item.items.map((it, i) => (
    ...
    case 'pkg-grid':
      return (
        <div key={idx} className="grid grid-cols-1 sm:grid-cols-2 gap-2 my-4">
    ...
    case 'chain-grid':
      return (
        <div key={idx} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 my-4">
    ...
    case 'track': {
      ...
      return (
        <div key={idx} className={`border-l-2 border border-[#1E1E1E] px-4 pt-3 pb-1 my-3 ${box}`}>
...
function Section({ section }: { section: typeof DOCS[number] }) {
  return (
    <section>
...
export default function DocsPage() {
  const overview = DOCS.find(s => s.id === 'overview')
  const overviewText = overview?.items[0]?.type === 'text' ? overview.items[0].md : ''
    ...
    <div className="font-body text-muted">
      ...
      <div id="overview" className="mb-10 scroll-mt-20">
        ...
        <p className="text-[15px] text-muted leading-relaxed max-w-xl">
          {parseInline(overviewText)}
        </p>
      </div>

      {/* All sections (skip overview — rendered as hero above) */}
      {DOCS.filter(s => s.id !== 'overview').map(section => (
